...
process: cash flow (2.1) and financial planning / budgeting (2.2)
stage: first hires through early revenue (2–30 people)
date: 2026-06-24
source_note: Practitioner experience. Scenario-style planning framing attributed to Nalden (operator). Corona mass-cancellation prep anecdote from Stefan's leadership-team experience. No external URL — primary source is first-person / attributed practitioner knowledge.
attribution:
...
Nalden gave it a name I keep using: **IFTTT** — budget **and** bandwidth **and** if/then actions, IFTTT-style, like the workflow tool *If This Then That*. You set the line. You set what happens when you cross it. So you're not inventing cuts or hires in a crisis meeting with adrenaline and Slack threads.
...
- Every planned hire has a **trigger**, not just a calendar date — e.g. *We hire the next developer when ARR passes €X and the hire still leaves us above Y months runway after fully loaded cost — currently pencilled for April, not locked for April.*
- Material spend has **pre-agreed responses** when a named warning fires — not a vague "we'll tighten up."
- Founders **already argued** the downside in a calm month — execution in a bad month is follow-the-sheet.
- Leadership knows which numbers are watched and what happens at each threshold — no surprise *we're cutting tomorrow* without context.

**Contrast with bad:** Hire dates on January 1 and July 1. Revised only when cash gets tight — or when an investor forces the conversation. Slow death or reactive whiplash. Same failure mode, different tempo.
...
- **Cash tab** — opening balance, monthly burn, runway in months. Update monthly; weekly if cash is tight. (See **2.1** — keep aligned with cumulative cash on the main tab.)
- **Monthly founder review (15–30 min)** — actual vs plan, runway to the nearest month, which triggers are close or have fired, next hire gate status.

Clean expense categories live in **2.4 Accounts Payable & Expense Management** — without them, budget vs actual is fiction.

### 2. Trigger → action (one list, many dimensions)
...
|-------------------|----------------------------------|
| MRR ≥ €X for two consecutive months | Hire next developer; refresh cash plan with fully loaded cost |
| Paying customers > N and support load sustained | Hire dedicated support (not "when we feel overwhelmed") |
...
- **Static budget only** — hire dates on the calendar, no link to revenue or triggers.
- **Triggers live in founders' heads** — you "know" you'll cut freelancers, but disagree on when the moment hits.
- **Optimistic revenue in the base case** — downside never named (plan at zero revenue for survival math).
- **Bank line counted in runway** — facility hides how fast you burn through both.
- **Budget vs actual without categories** — variance review is useless; see **2.4**.
- **Only the CEO holds the numbers** — everyone else optimizes locally, burn compounds collectively.

Warning signs you are behind:

- Can't answer *what happens to cash if we hire X next month?* within a working day.
- Last cuts conversation was **after** something broke.
- Every hire debate is moral (*we need this person*) not arithmetic (*trigger met or not*).

## How does this change as you grow?
...
- **Google Sheet** — base plan + trigger table (metric | threshold | action | owner | last reviewed).
...
| Wiki | Process |
|------|---------|
| [2.1 Cash Flow — First Hires](https://aksie.github.io/ducttape-to-coo/wiki.html#2.1--first-hires) | Cash tab, runway math, weekly review when tight |
| [2.2 Financial Planning — First Hires](https://aksie.github.io/ducttape-to-coo/wiki.html#2.2--first-hires) | Headcount budget, trigger → action list, monthly review |
| [2.4 AP & Expense — First Hires](https://aksie.github.io/ducttape-to-coo/wiki.html#2.4--first-hires) | Categories and accountant — foundation for variance |
| [1.4 Who Decides What](https://aksie.github.io/ducttape-to-coo/wiki.html#1.4--first-hires) | Spend/hire approval list connects to playbook |
...
- **Stage caveat:** At foundation (no payroll), **2.1** weekly cash only; scenario rules start once payroll commitments exist.
- **Uncertainty:** Threshold numbers vary by funding model — flag `needs_practitioner_check` at synthesis.

## Pipeline mapping (for maintainer)

| Target | Likely claims |
|--------|----------------|
| **2.2 first hires** | Scenario-style plan vs static budget; monthly founder review; trigger → action list (multi-metric) |
| **2.1 first hires** | Cash plan + runway math; weekly review when cash is tight; don't count debt line in core runway |
| **1.2b first hires** | Investor-facing scenario bullets mirror internal trigger sheet |
| **1.4 first hires** | Spend/hire approval list connects to trigger playbook |

Suggested source id when registered: `src-065` or next free — `extracted_by: human:stefanverkerk`; attribute Nalden framing in source summary and trail.

## Phase 1b extraction (2026-06-24)
